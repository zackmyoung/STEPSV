...
# -*- coding: utf-8 -*-
"""
ML_offset_detect_v151_composite.py (composite)

Composite script that combines:
- Dataset build + save (from ML_offset_detect_v91/v99)
- Training with folds (from v91)
- Final training on full dataset (from v99)
- Inference (from test_ml_step_eval_v99_final)

Goal: keep functionality identical to the original scripts, but provide a single entrypoint.

# Created: 2026-01-22
"""

# ============================================================
# Shared training code (verbatim prefix from ML_offset_detect_v99.py)
# ============================================================
#!/usr/bin/env python
# coding: utf-8
...
import argparse
import os
import time  # added for dataloader progress/ETA
import pickle
...
def ddp_cleanup(state: _DDPState):
    if state.enabled:
        dist.barrier()
...
def ddp_print(state: _DDPState, *a, **k):
    if (not state.enabled) or state.is_main:
        print(*a, **k)

def _ddp_allreduce_sum_tensor(t: torch.Tensor, state: _DDPState) -> torch.Tensor:
    if state.enabled:
        dist.all_reduce(t, op=dist.ReduceOp.SUM)
    return t
...
# ----------------------------------------------------------------------
# Simple progress logger (optional file)
# ----------------------------------------------------------------------
def _log(msg, log_path=None):
    """Print + optionally append to a log file (line-buffered)."""
